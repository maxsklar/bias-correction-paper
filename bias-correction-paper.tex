...
\begin{enumerate}
	...
	\item Each predictor \(h \in H\) is parameterized by \(h = (c, \mathbf{w})\) where \(c \in \mathbb{R}\) is the intercept and \(\mathbf{w} \in \mathbb{R}^{|F|}\) is an \(|F|\)-dimensional vector of weights corresponding to each feature.
          \item Each hypothesis \((c, \mathbf{w}) \in H\) corresponds to the following PDF, given here in both normalized and unnormalized form.
          \begin{align}
          \hat{f}_{c,\mathbf{w}}(x_n, y_n)=\frac{e^{y_n \cdot (c+\mathbf{w} \cdot x_n)}}{1+e^{c+\mathbf{w} \cdot x_n}} &
          \qquad f_{c,\mathbf{w}}(x_n, y_n)=e^{y_n \cdot (c+\mathbf{w} \cdot x_n)}
         \end{align}
\end{enumerate}
...
\begin{align}
\mathbf{P}(y_n=1|x_n,h,\mathbf{s})=\frac{e^{c+\mathbf{w} \cdot x_n}\mathbf{s}(x_n,1)}{\mathbf{s}(x_n,0)+e^{c+\mathbf{w} \cdot x_n}\mathbf{s}(x_n,1)}=\frac{e^{c+\mathbf{w} \cdot x_n}}{\mathbf{s}_r(x_n)+e^{c+\mathbf{w} \cdot x_n}} &
